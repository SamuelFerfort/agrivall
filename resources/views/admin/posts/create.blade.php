                    ...
                    <form action="{{ route('admin.posts.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <select id="tipo_post_id" name="tipo_post_id" required
                                ...
                                @foreach ($tiposPost as $tipo)
                                    <option value="{{ $tipo->id }}" {{ old('tipo_post_id') == $tipo->id ? 'selected' : '' }}>
                                        {{ $tipo->tipo }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="noticia" name="noticia" rows="6" required
                                      class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('noticia') }}</textarea>
                        ...
                        <div class="flex items-center gap-4">
                            <x-primary-button>Crear Post</x-primary-button>
